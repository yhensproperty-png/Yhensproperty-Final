  ...
  return (
    <div className="bg-white dark:bg-zinc-900 rounded-2xl shadow-sm border border-zinc-100 dark:border-zinc-800 hover:shadow-xl transition-all flex flex-col h-full group">
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <div className={`grid ${isLand ? 'grid-cols-2' : 'grid-cols-4'} gap-4 pt-4 border-t border-zinc-100 dark:border-zinc-800`}>
          {!isLand && (
            <>
        ...
        <div className="flex gap-2 pt-4 mt-4 border-t border-zinc-100 dark:border-zinc-800">
          <button
            onClick={handleWhatsApp}
            className="flex-1 flex items-center justify-center gap-2 bg-[#25D366] hover:bg-[#20BA5A] text-white font-bold py-2.5 px-4 rounded-xl transition-all shadow-sm hover:shadow-md active:scale-95"
          >
          ...
          <button
            ...
            className="flex-1 flex items-center justify-center gap-2 bg-[#0084FF] hover:bg-[#0073E6] text-white font-bold py-2.5 px-4 rounded-xl transition-all shadow-sm hover:shadow-md active:scale-95"
          >
            <svg className="w-4 h-4 fill-white flex-shrink-0" viewBox="0 0 24 24">
...
const Home: React.FC<HomeProps> = ({ properties, isLoading }) => {
      ...
      let results = activeProperties.filter(p => {
        ...
        if (searchFilters.beds) {
          if (searchFilters.beds === '5+') {
            ...
            const exactBeds = parseInt(searchFilters.beds);
            if (p.beds !== exactBeds) return false;
          }
        }
        ...
        if (searchFilters.baths) {
          if (searchFilters.baths === '5+') {
            ...
            const exactBaths = parseFloat(searchFilters.baths);
            if (p.baths !== exactBaths) return false;
          }
        }
  ...
  return (
    <div>
      <SEO
        title="Luxury Condos, Houses & Commercial in Philippines for Locals & Foreigners | Yhen's Property – BGC, Manila, Batangas"
        description="Licensed agent helping Filipinos and foreigners buy or rent condos, houses, land and commercial properties across the Philippines. Direct deals, legal guidance for expats, focus on BGC, Manila and Batangas"
